...
CSearchData::CSearchData(ifstream &input)
{
	...
	for (size_t i = 0; i < m_fieldSize; i++)
	{
		...
		for (size_t j = 0; j < m_fieldSize; j++)
		{
			...
			m_coordinates.insert(CellCoordinates::value_type(cell, Point((short)j, (short)i)));
		}
...
void CSearchData::SetPath(const vector<short> &path)
{
	m_path = path;
}
...
void CSearchData::Print(ostream &output)
{
	...
	output << "Memory usage: " << CalculateMemoryInMB() << " MB" << endl;
...
float CSearchData::CalculateMemoryInMB()
{
	PROCESS_MEMORY_COUNTERS pmc;
	GetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc));
	float physMemUsage = static_cast<float>(pmc.WorkingSetSize);
	return physMemUsage / 1024 / 1024;
}
